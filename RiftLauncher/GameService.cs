...
namespace RiftLauncher
{
	...
	public class GameService
	{
		...
		{
			return File.Exists($"{cfg.GameInstallLocation}{version}\\Insurrection.exe") || File.Exists($"{cfg.GameInstallLocation}{version}\\Rift.exe");
		}
		...
		public async Task StartGameProcess(SemVersion semVer)
		{
			...
			string filename = "";
			if (File.Exists($"{cfg.GameInstallLocation}{semVer}\\Insurrection.exe") )
				filename = "Insurrection.exe";
			if (File.Exists($"{cfg.GameInstallLocation}{semVer}\\Rift.exe"))
				filename = "Rift.exe";
		...
		private void DecompressToDirectoryAsync(string path, string target, SemVersion semVer)
		{
			...
			else
			{
				...
				//{
				//	root = zip.Entries.First().FullName.Split("\\")[0];
