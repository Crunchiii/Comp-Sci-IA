...
namespace RiftLauncher
{
	...
	{
		private const string _releasesEndpoint = "https://api.github.com/repos/Fabeuro/Rift_Builds/releases";
		...
		public GithubService()
		{
			...
			_httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("RiftLauncher", Assembly.GetExecutingAssembly().GetName().Version.ToString()));
			_webClient = new WebClient();
			_webClient.Headers.Add("User-Agent", $"RiftLauncher-{Assembly.GetExecutingAssembly().GetName().Version}");
		...
		private string GetNextPageNumber(HttpHeaders headers)
		{
			...
			return !links.Any() ? null : (from link in links where link.Contains(@"rel=""next""")
